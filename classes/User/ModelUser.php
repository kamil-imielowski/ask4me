...
include_once dirname(dirname(dirname(__FILE__))).'/functions/validate.php';
...
class ModelUser extends User{
    ...
    function withdrawTokens($data){
        ...
        $date = date("Y-m-d H:i:s");
        $translate = new \classes\Languages\Translate($_COOKIE['lang']);
        $errors = validate("IBAN", $data['iban'], 'nrb');
        if(!empty($errors)){
            throw new \Exception($translate->getString('ibanIncorrect'));
        }
